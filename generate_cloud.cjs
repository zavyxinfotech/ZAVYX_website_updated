...
function CloudInfrastructureView() {
  ...
  const faqs = [
    { q: 'What cloud platforms do you work with?', a: 'We specialize in AWS, Microsoft Azure, and Google Cloud Platform (GCP).' },
    { q: 'Can you help optimize our existing cloud setup?', a: 'Yes, we provide comprehensive cloud audits to identfy cost savings and performance improvements.' },
    { q: 'How long does a typical migration take?', a: 'Depending on the complexity, migrations can span from a few weeks to several months.' },
    { q: 'Do you provide ongoing support?', a: 'Absolutely, we offer 24/7 monitoring, incident response, and proactive maintenance.' },
    { q: 'Is my data secure in the cloud?', a: 'We integrate industry-leading security practices and compliance standards into all deployments.' },
    { q: 'What industries do you serve?', a: 'We serve diverse industries including healthcare, finance, tech startups, and e-commerce.' }
  ];
  ...
  return (
    <div className="bg-slate-50/50 dark:bg-[#050B14] text-slate-900 dark:text-slate-50 min-h-screen transition-colors duration-300 font-sans pb-16 relative overflow-hidden">
      ...
      <section className="py-12 sm:py-16 max-w-[1350px] mx-auto px-6 sm:px-8 lg:px-12 relative z-10 mt-12 bg-sky-50/50 dark:bg-slate-900/50 rounded-3xl border border-slate-200/60 dark:border-slate-800/60">
      ...
      {/* 8. FAQ SECTION */}
      <section className="py-12 sm:py-16 max-w-[1350px] mx-auto px-6 sm:px-8 lg:px-12 relative z-10 mb-8 pt-10 border-t border-slate-200/60 dark:border-slate-800/60">
        <ScrollSlideSection direction="up" className="mb-10 flex flex-col justify-between items-start lg:flex-row lg:items-end">
          ...
          <p className="text-slate-600 dark:text-slate-400 text-sm mt-4 lg:mt-0 lg:text-right">
            Still have questions? We\\'re here to help. <br/>
            <Link to="/contact" className="text-sky-600 dark:text-sky-400 inline-flex items-center gap-1 group">Contact Us <ArrowRight className="w-3 h-3 group-hover:translate-x-1 transition-transform" /></Link>
          </p>
        </ScrollSlideSection>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-6">
           {faqs.map((faq, idx) => (
             <ScrollSlideSection key={idx} delay={idx*50 + "ms"} direction="up" className="border-b border-slate-200 dark:border-slate-700/60 pb-5 text-left">
                <div className="flex justify-between items-start gap-4 group cursor-pointer">
                   <h3 className="text-base sm:text-lg font-normal text-slate-900 dark:text-white group-hover:text-sky-600 transition-colors duration-200">{faq.q}</h3>
...
code = code.substring(0, exportIdx) + cloudView + "\n\n" + code.substring(exportIdx);

if (!code.includes("currentServiceKey === 'cloud-infrastructure'")) {
  code = code.replace(
...
if (!code.includes("import cloudBgImg")) {
  const lastImport = code.lastIndexOf("import ");
  const endOfLastImport = code.indexOf(";\n", lastImport) + 2;
  code = code.substring(0, endOfLastImport) + "import cloudBgImg from '../../assets/images/Cloud_Infrastructure_service_background.png';\n" + code.substring(endOfLastImport);
}

// Ensure icons like Cloud, Server, Infinity, Maximize, Activity, DollarSign, Clock, RefreshCw are imported from lucide-react
const importLucideLine = code.match(/import {.*} from 'lucide-react';/)[0];
let newImportLucide = importLucideLine;
['Cloud', 'Server', 'Infinity', 'Maximize', 'Activity', 'DollarSign', 'Clock', 'RefreshCw'].forEach(icon => {
...
console.log("CloudInfrastructureView injection complete.");
